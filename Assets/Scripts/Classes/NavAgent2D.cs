    ...
    public void SetDestination(Vector3 targetdestination) //linear movement
    {
        destination = targetdestination;
            ...
            //path.direction = new Vector2(destination.x, destination.y); //create a 2d ray intersecting the origin and the target destination
                                                                        //Vector3
            gameObject.transform.position = Vector3.MoveTowards(transform.position, destination, Time.deltaTime * velocity);//Vector2.MoveTowards(transform.position, path.direction, Time.deltaTime * velocity);
                                                                                                                                  //transform.position = Vector2.MoveTowards(transform.position, destination2D, Time.deltaTime * velocity);
